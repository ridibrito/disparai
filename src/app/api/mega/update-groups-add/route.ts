// API para atualizar configuração de adicionar a grupos
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${MEGA_HOST}/rest/privacy/${instanceKey}/updateGroupsAdd`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: result.message || 'Erro ao atualizar configuração de grupos' },

// API para limpar chat (deletar todas as mensagens)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${MEGA_HOST}/rest/chat/${instanceKey}/clearChat`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: result.message || 'Erro ao limpar chat' },

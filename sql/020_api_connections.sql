-- Schema para sistema de conexões API
-- Execute: psql "$DATABASE_URL" -f sql/020_api_connections.sql

-- Tabela para conexões API (substitui api_connections existente se necessário)
CREATE TABLE IF NOT EXISTS public.api_connections (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES public.users(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
  type TEXT NOT NULL CHECK (type IN ('whatsapp_cloud', 'whatsapp_disparai')),
  phone_number TEXT, -- Para WhatsApp Cloud API
  instance_id TEXT, -- Para WhatsApp Disparai
...
-- Tabela para logs de uso das conexões
...
-- Política: sistema pode inserir logs (para webhooks, etc.)

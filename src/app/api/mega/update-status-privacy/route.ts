// API para atualizar privacidade do status
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${MEGA_HOST}/rest/privacy/${instanceKey}/updateStatus`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: result.message || 'Erro ao atualizar privacidade do status' },

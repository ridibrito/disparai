...
CREATE TABLE IF NOT EXISTS ai_agents (
    ...
    type VARCHAR(100) NOT NULL, -- 'sdr', 'atendimento', 'vendas', 'suporte', etc.
    description TEXT,
...
-- Criar tabela de configurações de agentes por instância
CREATE TABLE IF NOT EXISTS agent_instance_configs (
    ...
    escalation_rules JSONB, -- Regras para quando escalar para humano
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    UNIQUE(whatsapp_instance_id, agent_id)
);

-- Criar tabela de histórico de respostas automáticas
...
-- Políticas RLS para agent_instance_configs
...
-- Políticas RLS para agent_responses

...
export function CampaignsTable({ initialCampaigns }: CampaignsTableProps) {
  ...
  }, [initialCampaigns, search, status]);
        ...
        <>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              ...
              <tbody>
                {pageItems.map((c) => (
                  <tr key={c.id} className="border-b border-[var(--color-border)] hover:bg-[var(--color-background)]">
                    <td className="py-3 px-4">{c.name}</td>
                    <td className="py-3 px-4">{statusText(c.status)}</td>
                    <td className="py-3 px-4">{c.contact_count ?? 0}</td>
                    <td className="py-3 px-4">{new Date(c.created_at).toLocaleDateString('pt-BR')}</td>
                    <td className="py-3 px-4">
                      <div className="flex gap-2">
                        <Link href={`/disparos/${c.id}`}>
                          <Button variant="ghost" size="sm" className="h-8 px-3">Detalhes</Button>
                        </Link>
                      </div>
                    </td>
                  </tr>

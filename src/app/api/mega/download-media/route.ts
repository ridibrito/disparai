// API para baixar mídia de mensagem
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${MEGA_HOST}/rest/instance/downloadMediaMessage/${instanceKey}`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: result.message || 'Erro ao baixar mídia' },
